...
function generateShortcut() {
  ...
  const shortcutsInCategory = shortcuts[randomCategory];
  const randomShortcut =
    shortcutsInCategory[Math.floor(Math.random() * shortcutsInCategory.length)];

  // Display the random shortcut
  shortcutDisplay.textContent =
    randomShortcut.key + " - " + randomShortcut.description;
}
